          ...
          <PageTransition className={styles.settingPanelList} >
            <div className={classNames(styles.settingPanel)}>
              ...
              <div className={classNames(styles.avatar)}>
                <Icon src={userBasicInfo.avatar ?? avatarIcon.src} style={{ objectFit: 'cover' }} alt="avatar" width={145} height={145} />
              </div>
              <div className={styles.formContainer}>
                <Button onClick={() => { openFilePicker() }} className={classNames(styles.button)}>
                  <Icon {...cameraIcon} />
                  更换头像
                </Button>
              </div>
              ...
              <div className={classNames(styles.settingPanel)}>
                ...
                <div className={styles.formContainer}>
                  <InputWithLabel
                    ...
                    error={{ error: false }}
                    defaultValue={userBasicInfo.link === null ? '' : userBasicInfo.link[0]}
                  />
                  <InputWithLabel
                    className={classNames(styles.input)}
                    label={"社交链接"}
                    ...
                    error={{ error: false }}
                    defaultValue={userBasicInfo.link === null ? '' : userBasicInfo.link[1]}
                  />
                  <InputWithLabel
                    className={classNames(styles.input)}
                    label={"社交链接"}
                    ...
                    error={{ error: false }}
                    defaultValue={userBasicInfo.link === null ? '' : userBasicInfo.link[2]}
                  />
                </div>
                <div className={styles.formContainer}>
                  <Button loading={updateLoading} type="submit" className={classNames(styles.button)}>提交</Button>
                </div>
              </div>
            </form>
          </PageTransition>
          :
          <PageTransition style={{ alignItems: 'center' }} className={styles.settingPanelList}>
            <AvatarEditor
              image={avatarFile}
              width={200}
              height={200}
            ...
            />
            <input type="range" min={1} max={5} step={0.01} value={avatarScale} onChange={(value) => { setAvatarScale(Number(value.target.value)) }} />
            <Button loading={uploadLoading} onClick={handleAvatarUpdate}>确认提交</Button>
          </PageTransition>
      }
    </>
  );
